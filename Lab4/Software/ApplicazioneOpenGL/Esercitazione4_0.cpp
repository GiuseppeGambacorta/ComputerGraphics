...
float alpha=0.7; // variabile globale usata nelle inizializzazioni delle forme per cambiare la teasparenza
int i,j;
...
*/
    ...
    MatProj = glGetUniformLocation(programId, "Projection"); // deve avere lo stesso nome di quella messa nello shader
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        //lascio qui per fare animazioni
        Scena[0].Model = mat4(1.0);
        Scena[0].Model = translate(Scena[0].Model, vec3(width / 2.0, height / 2.0, 0.0f));
        Scena[0].Model = scale(Scena[0].Model, vec3(100.0f, 100.0f, 0.0f));
        ...
        Scena[1].Model = translate(Scena[1].Model, vec3(width / 2.0, height / 2.0 - 50, 0.0f));
        Scena[1].Model = scale(Scena[1].Model, vec3(100.0f, 100.0f, 0.0f));
